...
func ucdDispatcher(ctx *Context, r *http.Request) http.Handler {
	ucdHandler := ucdHandler{}
	ucdHandler.manifestHandler.Context = ctx
	reference := getReference(ctx)
	dgst, err := digest.Parse(reference)
	if err != nil {
		// We just have a tag
		ucdHandler.Tag = reference
	} else {
		ucdHandler.Digest = dgst
	}

	uhandler := handlers.MethodHandler{
		"GET":  http.HandlerFunc(ucdHandler.GetUCD),
		"HEAD": http.HandlerFunc(ucdHandler.GetUCD),
	}
...
func (uh *ucdHandler) GetUCD(w http.ResponseWriter, r *http.Request) {
